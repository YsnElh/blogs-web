...
function print_posts(){
    ...
    if (count($posts) > 0) {
        foreach($posts as $p){
            ...
            if (isset($_GET['search']) && $_GET['search'] == 'pending') {
                echo '<h3><a href="#" class="menu__link">'.$p['title'].'</a></h3>';
                echo '<div class="buttons-post">';
            }elseif(isset($_GET['search']) && $_GET['search'] == 'archived'){
                echo '<h3><a href="#" class="menu__link">'.$p['title'].'</a></h3>';
                echo '<div class="buttons-post">';
                echo '<a href="edit-post?post-id='.$p['id'].'" class="menu__link">Edit</a>';

            }else{
                echo '<h3><a href="/singlepost?id='.$p['id'].'" class="menu__link">'.$p['title'].'</a></h3>';
                echo '<div class="buttons-post">';
                echo '<a href="edit-post?post-id='.$p['id'].'" class="menu__link">Edit</a>';
            }
