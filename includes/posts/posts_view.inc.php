...
function not_found_posts(){
   global $posts;
   if(isset($_GET['page-nr']) && empty($posts)){
      echo '<h2 style="color:red">There is no blogs yet</h2>';
      return true;
   }
}
...
function print_posts(){
   global $posts;
    if (isset($posts) || isset($_GET['page-nr']) || isset($_GET['page-nr-s'])){
        foreach ($posts as $post) {     
            ...
            if (isset($post['category_names'])) {
               ...
               foreach (array_unique($post['category_names']) as $category) {
                  echo $category . '. ';
               }
            }
            echo '</span>';
